...
async function loadSubjects() {
  try {
    // Load exercises from SQL Server via /api/baitap
    const baitaps = await fetchJSON('/api/baitap');
    console.log('Loaded baitaps:', baitaps.length, 'exercises');
    
    // Transform flat baitap array into subjects/forms/exercises structure
    // Group by TenMon (subject), then by TenDangBai (form type)
    const subjectsMap = {};
    
    baitaps.forEach(b => {
      const tenMon = b.TenMon || 'Unknown Subject';
      const tenDangBai = b.TenDangBai || 'Unknown Type';
      const tenDoKho = b.TenDoKho || 'Unknown';
      
      if (!subjectsMap[tenMon]) {
        subjectsMap[tenMon] = {
          subject_id: tenMon,
          subject_name: tenMon,
          description: '',
      ...
      const subject = subjectsMap[tenMon];
      if (!subject.forms[tenDangBai]) {
        subject.forms[tenDangBai] = {
          form_id: tenDangBai,
          name: tenDangBai,
          difficulty: tenDoKho,
          exercise_count: 0,
      ...
      const form = subject.forms[tenDangBai];
      form.exercises.push({
        id: b.MaBaiTap,
        title: b.TenBaiTap,
        description: b.MoTa || '',
        ...
        grading_criteria: (() => {
          try {
            if (!b.TieuChiChamDiem) return [];
            const parsed = JSON.parse(b.TieuChiChamDiem);
            return (parsed && parsed.tieu_chi && Array.isArray(parsed.tieu_chi)) ? parsed.tieu_chi : [];
          } catch (e) {
            return b.TieuChiChamDiem ? [b.TieuChiChamDiem] : [];
          }
        })()
      });
      
      form.exercise_count = form.exercises.length;
      subject.total_exercises += 1;
    });
    
    // Convert map to array and ensure forms is an array
    state.subjects = Object.values(subjectsMap).map(s => {
      const formsArray = Object.values(s.forms);
      return {
    ...
    console.log('Transformed subjects:', state.subjects.length);
    renderSidebar();
    ...
    try {
      ...
      if (!hash && state.subjects && state.subjects.length) {
        console.log('Auto-selecting subject:', state.subjects[0].subject_id);
        await selectSubject(state.subjects[0].subject_id);
      }
    } catch (e) { console.error('Error auto-selecting:', e); }
  } catch (err) {
    console.error('Error loading subjects:', err);
    // Fallback: show error message
    const container = document.getElementById('forms-container');
    if (container) {
      container.innerHTML = `<div class="card" style="color:red;">Lỗi: Không thể tải dữ liệu bài tập. ${err.message}</div>`;
    }
  }
}
...
async function selectSubject(id) {
  // Lấy subject từ state.subjects array (không cần gọi API)
...
function showExercise(ex) {
  ...
  function renderGradingHtml(criteria) {
    // Handle JSON string, array, or object
    let items = [];
    
    if (typeof criteria === 'string') {
      try {
        const parsed = JSON.parse(criteria);
        // If parsed is object with tieu_chi field, use it
        if (parsed && parsed.tieu_chi && Array.isArray(parsed.tieu_chi)) {
          items = parsed.tieu_chi;
      ...
      items = criteria;
    } else if (criteria && criteria.tieu_chi && Array.isArray(criteria.tieu_chi)) {
      items = criteria.tieu_chi;
    }
    
    if (!items || !items.length) return '<div class="small muted">(Không có tiêu chí)</div>';
    
    return '<ol>' + items.map(g => {
      if (!g) return '<li>(Không xác định)</li>';
      if (typeof g === 'string') return `<li>${escapeHtml(g)}</li>`;
      const name = escapeHtml(g.name || g || '(Không tên)');
  ...
  d.innerHTML = `
    ...
    <div><strong>Yêu cầu:</strong><div style="margin-left:16px">${Array.isArray(ex.requirements) ? ex.requirements.map(r => `<div style="margin-bottom:8px">• ${escapeHtml(r)}</div>`).join('') : (ex.requirements ? `<div style="margin-bottom:8px">• ${escapeHtml(ex.requirements)}</div>` : '<div>(Không có)</div>')}</div></div>
